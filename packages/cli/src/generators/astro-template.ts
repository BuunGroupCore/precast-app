...
export async function generateAstroTemplate(config: ProjectConfig, projectPath: string) {
  ...
  try {
    ...
    // Copy base Astro templates
    await templateEngine.copyTemplateDirectory("frameworks/astro/base", projectPath, config, {
      overwrite: true,
    });
    ...
    consola.success("Astro project generated successfully!");
  } catch (error) {
    consola.error("Failed to generate Astro project:", error);
    throw error;
  }
}
