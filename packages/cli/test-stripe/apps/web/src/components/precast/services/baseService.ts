...
export abstract class AbstractServiceTester implements BaseServiceTester {
   ...
   */
  protected createTestResult(success: boolean, message: string, details?: unknown): TestResult {
    return {
      success,
      message,
      details,
  ...
  protected handleError(error: unknown, serviceName: string): TestResult {
    ...
    return this.createTestResult(false, `${serviceName.toUpperCase()} ERROR`, errorMessage);
  }
 ...
 */
export function registerServiceTester(serviceKey: string, factory: ServiceTesterFactory): void {
