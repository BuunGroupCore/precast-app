...
import { frameworkDefs, backendDefs, databaseDefs, ormDefs, stylingDefs } from "../../../shared/stack-config.js";
...
export async function gatherProjectConfig(
  ...
  // Get backend
  const backend = options.backend || await select({
  ...
  // Get styling
  const styling = options.styling || await select({
  ...
  const typescript = options.typescript !== undefined 
    ? options.typescript 
    : options.yes 
      ? true 
      : await confirm({
  ...
  const docker = options.yes ? false : (options.docker ?? await confirm({
