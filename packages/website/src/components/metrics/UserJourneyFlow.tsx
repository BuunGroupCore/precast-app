...
export function UserJourneyFlow(_props: UserJourneyFlowProps) {
  ...
  return (
    ...
    >
      <h3 className="font-display text-2xl mb-4 text-comic-yellow flex items-center gap-2">
        <FaRoute /> USER JOURNEY
      </h3>

      {/* Key Metrics */}
      <div className="grid md:grid-cols-4 gap-4 mb-6">
        <div className="comic-panel p-4 bg-comic-purple text-center">
          <FaLaptopCode className="text-2xl mx-auto mb-2 text-comic-white" />
          <div className="font-display text-sm text-comic-white">ENTRY POINT</div>
          <div className="action-text text-lg text-comic-white uppercase">
        ...
        <div className="comic-panel p-4 bg-comic-green text-center">
          ...
          <div className="font-display text-sm text-comic-white">COMPLETION</div>
        </div>
        ...
        <div className="comic-panel p-4 bg-comic-red text-center">
          <FaRedo className="text-2xl mx-auto mb-2 text-comic-white" />
          <div className="action-text text-2xl text-comic-white">{journey?.retryAttempts || 0}</div>
          <div className="font-display text-sm text-comic-white">RETRIES</div>
        </div>
      </div>
      ...
      {journey?.commonPaths && journey.commonPaths.length > 0 && (
        <div className="mb-6">
          <h4 className="font-display text-xl mb-4 text-comic-purple">COMMON PATHS</h4>
            ...
            {journey.commonPaths
              ...
              .map((path: { path: string[]; frequency: number }, index: number) => (
                <motion.div
                  ...
                  className="flex items-center justify-between p-3 rounded-lg bg-comic-purple bg-opacity-10 border-2 border-comic-purple"
                >
                  <div className="flex items-center gap-2">
                    <span className="action-text text-comic-purple">#{index + 1}</span>
                    <div className="flex items-center gap-2">
                      {path.path.slice(0, 3).map((step: string, stepIndex: number) => (
                        <div key={stepIndex} className="flex items-center gap-1">
                          <span className="comic-panel px-2 py-1 bg-comic-purple text-comic-white text-xs font-comic font-bold">
                            {step
                              .replace(/[_$]/g, " ")
                              .replace(/\b\w/g, (l: string) => l.toUpperCase())}
                          </span>
                          {stepIndex < Math.min(path.path.length, 3) - 1 && (
                            <FaArrowRight className="text-comic-purple" />
                          )}
                        </div>
                      ))}
                      {path.path.length > 3 && (
                        <span className="font-comic text-comic-black text-xs">
                          +{path.path.length - 3}
                        </span>
                      )}
                    </div>
                  </div>
                  <div className="action-text text-comic-red">{path.frequency}</div>
                </motion.div>
              ))}
          </div>
        </div>
      ...
      {journey?.dropoffPoints && journey.dropoffPoints.length > 0 && (
        <div>
          <h4 className="font-display text-xl mb-4 text-comic-red">DROPOFF POINTS</h4>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {journey.dropoffPoints.slice(0, 6).map((dropoff: string, index: number) => (
              <motion.div
                ...
                className="flex items-center gap-2 p-3 comic-panel bg-comic-red bg-opacity-10 text-center"
              >
                <FaExclamationCircle className="text-comic-red" />
                <span className="font-comic text-comic-black text-sm font-bold">
