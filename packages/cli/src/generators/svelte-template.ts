...
export async function generateSvelteTemplate(config: ProjectConfig, projectPath: string) {
  ...
  try {
    ...
    // Copy base Svelte templates
    await templateEngine.copyTemplateDirectory("frameworks/svelte/base", projectPath, config, {
      overwrite: true,
    });
    ...
    consola.success("Svelte project generated successfully!");
  } catch (error) {
    consola.error("Failed to generate Svelte project:", error);
    throw error;
  }
}
