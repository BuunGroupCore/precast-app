...
function getApiClientPackages(apiClient: string, framework: string): ApiClientOption | null {
  const apiClients: Record<string, ApiClientOption> = {
    ...
    "hono-rpc": {
      ...
      devPackages: ["@types/node"],
      setupFiles: getHonoRPCSetupFiles(framework),
    },
  };
...
/**
 * Get Hono RPC setup files
 */
function getHonoRPCSetupFiles(framework: string): ApiClientOption["setupFiles"] {
  const files: ApiClientOption["setupFiles"] = [];

  // Client setup for frontend
  if (["react", "next", "vue", "svelte", "solid"].includes(framework)) {
    files.push({
      path: "src/lib/api-client.ts",
      content: `import { hc } from 'hono/client';
...
`,
    ...
    // Add React hooks for better DX
    if (["react", "next"].includes(framework)) {
      files.push({
        path: "src/hooks/use-api.ts",
        content: `import { useState, useEffect } from 'react';
...
`,
  ...
  // Server setup for backend (example API routes)
  files.push({
