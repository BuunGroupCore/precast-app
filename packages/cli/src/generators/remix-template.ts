...
export async function generateRemixTemplate(config: ProjectConfig, projectPath: string) {
  ...
  try {
    ...
    // Copy base Remix templates
    await templateEngine.copyTemplateDirectory("frameworks/remix/base", projectPath, config, {
      overwrite: true,
    });
    ...
    consola.success("Remix project generated successfully!");
  } catch (error) {
    consola.error("Failed to generate Remix project:", error);
    throw error;
  }
}
