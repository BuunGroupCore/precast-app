...
export async function generateVanillaTemplate(config: ProjectConfig, projectPath: string) {
  ...
  try {
    ...
    // Copy base Vanilla templates
    await templateEngine.copyTemplateDirectory("frameworks/vanilla/base", projectPath, config, {
      overwrite: true,
    });
    ...
    consola.success("Vanilla project generated successfully!");
  } catch (error) {
    consola.error("Failed to generate Vanilla project:", error);
    throw error;
  }
}
