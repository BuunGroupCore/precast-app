...
export async function generateEnvFiles(config: ProjectConfig): Promise<void> {
  try {
    ...
    if (isMonorepo) {
      // For monorepo: generate separate env files for web and api
      ...
      if (await fs.pathExists(webPath)) {
        ...
        // web/.env.development
        const webDevContent = formatEnvFile(frontendVars, "development");
        await fs.writeFile(path.join(webPath, ".env.development"), webDevContent);

        // web/.env.production
        const webProdContent = formatEnvFile(frontendVars, "production");
        await fs.writeFile(path.join(webPath, ".env.production"), webProdContent);
        ...
        const webExampleContent = formatEnvFile(webExampleVars, "development")
          .replace(
      ...
      if (await fs.pathExists(apiPath)) {
        ...
        // api/.env.development
        const apiDevContent = formatEnvFile(backendVars, "development");
        await fs.writeFile(path.join(apiPath, ".env.development"), apiDevContent);

        // api/.env.production
        const apiProdContent = formatEnvFile(backendVars, "production");
        await fs.writeFile(path.join(apiPath, ".env.production"), apiProdContent);
        ...
        const apiExampleContent = formatEnvFile(apiExampleVars, "development")
          .replace(
            "# Environment variables for development",
      ...
      consola.success("Generated scoped environment files:");
      if (await fs.pathExists(webPath)) {
        consola.info("  📁 web/.env.development, web/.env.production, web/.env.example");
      }
      if (await fs.pathExists(apiPath)) {
        consola.info("  📁 api/.env.development, api/.env.production, api/.env.example");
      }
    } else {
      // For single package projects: generate at root
      const allVars = envConfig.variables;

      // .env.development
      const devContent = formatEnvFile(allVars, "development");
      await fs.writeFile(path.join(config.projectPath, ".env.development"), devContent);

      // .env.production
      const prodContent = formatEnvFile(allVars, "production");
      await fs.writeFile(path.join(config.projectPath, ".env.production"), prodContent);
      ...
      const exampleContent = formatEnvFile(exampleVars, "development")
        .replace("# Environment variables for development", "# Example environment variables")
        .replace(/=.*/g, (match) => {
          const key = match.split("=")[0];
          if (key === "NODE_ENV") return "=development";
          return "=";
      ...
      consola.success(
        "Generated environment files: .env.development, .env.production, .env.example"
      );
    }
