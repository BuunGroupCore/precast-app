 ...
 * @param length - Length of the password to generate (default: 32)
 * @returns A random password string containing alphanumeric characters
 */
function generatePassword(length: number = 32): string {
  // Use only alphanumeric characters to avoid shell/YAML escaping issues
  // This is still secure with sufficient length (32 chars = ~190 bits of entropy)
...
export async function setupDockerCompose(
  ...
  try {
    ...
    // No need to encode alphanumeric passwords
