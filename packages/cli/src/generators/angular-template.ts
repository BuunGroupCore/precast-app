...
export async function generateAngularTemplate(config: ProjectConfig, projectPath: string) {
  ...
  try {
    ...
    // Copy base Angular templates
    await templateEngine.copyTemplateDirectory("frameworks/angular/base", projectPath, config, {
      overwrite: true,
    });
    ...
    consola.success("Angular project generated successfully!");
  } catch (error) {
    consola.error("Failed to generate Angular project:", error);
    throw error;
  }
}
