...
export async function generateSolidTemplate(config: ProjectConfig, projectPath: string) {
  ...
  try {
    ...
    // Copy base Solid templates
    await templateEngine.copyTemplateDirectory("frameworks/solid/base", projectPath, config, {
      overwrite: true,
    });
    ...
    consola.success("Solid project generated successfully!");
  } catch (error) {
    consola.error("Failed to generate Solid project:", error);
    throw error;
  }
}
